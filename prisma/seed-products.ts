...
const DEVOCIONAL_DAYS: DevocionalDayData[] = [
  {
    dayNumber: 1,
  ...
  for (const day of DEVOCIONAL_DAYS) {
    await prisma.devocionalDay.create({
      data: {
        ...
        questions: day.questions as unknown as object,
...
async function seedKit() {
  ...
  const product = await prisma.product.create({
    data: {
      ...
      slug: SLUG_KIT,
      type: 'kit',
      price: new Decimal(PRICE_KIT),
      badge: 'Kit completo',
      tag: 'kit',
      description:
        'Tudo que você precisa para organizar e fortalecer sua família na fé: plano devocional semanal, guia de culto doméstico, planner familiar cristão, artes para imprimir e colocar em casa, versículos decorativos e capa para perfil "Minha família está em conserva". Prático e acessível.',
      active: true,
    },
  });
...
async function seedCurso() {
  ...
  const product = await prisma.product.create({
    data: {
      ...
      slug: SLUG_CURSO,
      type: 'curso',
      price: new Decimal(PRICE_CURSO),
      badge: 'Curso',
      tag: 'curso',
      description:
        'Curso curto para pais: cultura vs princípios bíblicos, educação dos filhos com base em Provérbios, autoridade e amor equilibrados, e como criar um ambiente espiritual saudável no lar. Conteúdo prático e aplicável.',
      active: true,
    },
  });
  ...
  await prisma.courseLesson.createMany({
    data: [
      ...
      },
      {
        ...
        summary:
          'A Bíblia apresenta verdades atemporais sobre casamento, filhos, autoridade e amor. Revisamos esses princípios e por que eles "blindam" a família quando aplicados com graça e sabedoria.',
    ...
    data: [
      {
        ...
        summary:
          'O versículo "instrui o menino no caminho em que deve andar" não é uma fórmula mágica, mas um chamado à responsabilidade e à perseverança. Refletimos sobre o que significa "caminho", como instruir na prática e como confiar no resultado a longo prazo.',
      ...
      },
      {
        ...
        summary:
          'Provérbios fala de vara, disciplina e correção. Como aplicar isso hoje sem violência e com amor? Abordamos a diferença entre punição e disciplina formativa, e como corrigir de forma que edifique.',
    ...
    data: [
      {
        ...
        summary:
          'Deus delegou aos pais a autoridade para proteger e orientar os filhos. Essa autoridade é serviço, não domínio. Refletimos sobre Efésios 6 e como exercer liderança sem provocar ira.',
      ...
      },
      {
        ...
        summary:
          'O amor no casamento (Efésios 5) e na relação com os filhos (Efésios 6) é o contexto da autoridade. Amor sacrificial e disciplina amorosa andam juntos. Como equilibrar os dois no dia a dia.',
    ...
    data: [
      {
        ...
        summary:
          'O culto doméstico não precisa ser longo ou complicado. Um momento de leitura, oração e conversa já transforma o ambiente. Ideias práticas para começar ou renovar esse hábito.',
      ...
      },
      {
        ...
        summary:
          'O ambiente espiritual é feito de conversas que apontam para Deus e de oração que inclui os filhos. Como falar de fé no dia a dia e como orar juntos em momentos de alegria e de dificuldade.',
...
const BADGES_DEVOCIONAL = [
  { code: 'primeiro_passo', name: 'Primeiro passo', description: 'Você deu o primeiro passo na jornada do devocional.', conditionType: 'days_completed' as const, conditionValue: { days: 1 } },
  { code: 'em_marcha', name: 'Em marcha', description: 'Cinco dias de reflexão em família.', conditionType: 'days_completed' as const, conditionValue: { days: 5 } },
  { code: 'dez_dias_bencao', name: 'Dez dias de bênção', description: 'Uma década de dias dedicados ao lar.', conditionType: 'days_completed' as const, conditionValue: { days: 10 } },
  { code: 'meio_caminho', name: 'Meio caminho', description: 'Metade da jornada concluída.', conditionType: 'days_completed' as const, conditionValue: { days: 15 } },
  { code: 'quase_la', name: 'Quase lá', description: 'Faltam apenas dez dias para o fim.', conditionType: 'days_completed' as const, conditionValue: { days: 20 } },
  { code: 'reta_final', name: 'Reta final', description: 'Últimos passos até os 30 dias.', conditionType: 'days_completed' as const, conditionValue: { days: 25 } },
  { code: 'guardiao_lar', name: 'Guardião do Lar', description: 'Completou os 30 dias do devocional Família em Conserva.', conditionType: 'days_completed' as const, conditionValue: { days: 30 } },
  { code: 'aquecendo', name: 'Aquecendo', description: 'Três dias seguidos no devocional.', conditionType: 'streak' as const, conditionValue: { days: 3 } },
  { code: 'fiel_jornada', name: 'Fiel na jornada', description: 'Manteve um streak de 7 dias seguidos no devocional.', conditionType: 'streak_7' as const, conditionValue: {} },
  { code: 'duas_semanas_firmes', name: 'Duas semanas firmes', description: 'Quatorze dias de sequência.', conditionType: 'streak' as const, conditionValue: { days: 14 } },
];

async function seedBadges(devocionalProductId: string) {
